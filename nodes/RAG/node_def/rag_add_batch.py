...
class RagAddBatch(NodeInterface):
    """
    输入参数:
    ...
    def get_init_node_data() -> NodeData:
        #基本信息
        node_data = NodeData()
        node_data.type = "RAG_ADD_BATCH"
        node_data.label = "向rag数据库中添加多条数据"
        node_data.desc = "向指定rag数据库中添加多条数据"
        ...
        param2.name = "dataList"
        param2.type = ParamType.list
        param2.typeDef = "List<{key:String, content:String, labels:List<String>}>\n其中 key为需要嵌入搜索的句子, content为插入内容, labels为数据标签"
        param2.desc = "需要被插入的数据"
        param2.require = True
