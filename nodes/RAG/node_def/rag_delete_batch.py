...
class RagDeleteBatch(NodeInterface):
    """
    输入参数:
    ...
    async def run(context:NodeContext, ws: WebSocketAgent) -> ReturnParam:
        ...
        try:
            ...
            rag_data.rag_service.delete_data(context.input_map["idList"].value)
    ...
    def get_init_node_data() -> NodeData:
        #基本信息
        node_data = NodeData()
        node_data.type = "RAG_DELETE_BATCH"
        node_data.label = "从rag数据库删除多条数据"
        ...
        param2.typeDef = "List<Long>"
        param2.desc = "需要被删除的数据id列表"
        param2.require = True
